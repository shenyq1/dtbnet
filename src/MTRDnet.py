...
class MTRDnet(nn.Module):
    def __init__(self,
                 in_channels: int = 1,
                 num_classes: int = 2,
                 bilinear: bool = True,
                 base_c: int = 64):
        super(MTRDnet, self).__init__()
        self.in_channels = in_channels
        self.num_classes = num_classes
        self.bilinear = bilinear
        ...
        self.up4 = Up(base_c * 2, base_c, bilinear)
        self.out_conv = OutConv(base_c, num_classes)
        self.unet = UNet(in_channels=8, num_classes=2, base_c=32)
...
#layer1
        ...
        x1 = self.ptconv0(torch.cat([x11, x21], dim=1))#2base --- base32
        midlayer.append(x1)
#layer2
        x11 = self.down1(x11)
        ...
        x2 = self.ptconv1(torch.cat([x12, x22], dim=1))#`4base --- base64`
        midlayer.append(x2)
#layer3
        x12 = self.down2(x12)
...
#layer5
        ...
        logits2 = torch.cat([logits, x_ori], dim=1)
