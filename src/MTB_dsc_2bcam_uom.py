...
class SelfAttention(nn.Module):
    def __init__(self, in_channels):
        ...
        self.conv_query = nn.Conv2d(in_channels, 2, kernel_size=1)
        self.conv_key = nn.Conv2d(in_channels, 2, kernel_size=1)
        self.conv_value = nn.Conv2d(in_channels, 4, kernel_size=1)
...
class Cross_SelfAttention(nn.Module):
    def __init__(self, in_channels):#in_channels实际是输入特征图的通道数的一半
        ...
        self.conv_query = nn.Conv2d(in_channels, 2, kernel_size=1)
        self.conv_key = nn.Conv2d(in_channels, 2, kernel_size=1)
        self.conv_value = nn.Conv2d(in_channels, in_channels//4, kernel_size=1)
    ...
    def forward(self, x):
        ...
        x1 = x[:,0:n,:,:]
        #x1池化
        x1 = F.max_pool2d(x1, kernel_size=4, stride=4)
        x2 = x[:,n:,:,:]
        #x2池化
        x2 = F.max_pool2d(x2, kernel_size=4, stride=4)
        # Compute query, key, and value
        ...
        # Apply attention to the value
        out11 = torch.bmm(value1.view(batch_size, -1, height * width), attn1.permute(0, 2, 1))#(b,n/4,h*w)*(b,h*w,h*w)->(b,n,h*w)
        out11 = out11.view(batch_size, channels//4, height, width)
        out12 = torch.bmm(value2.view(batch_size, -1, height * width), attn1.permute(0, 2, 1))
        out12 = out12.view(batch_size, channels//4, height, width)
        out21 = torch.bmm(value1.view(batch_size, -1, height * width), attn2.permute(0, 2, 1))
        out21 = out21.view(batch_size, channels//4, height, width)
        out22 = torch.bmm(value2.view(batch_size, -1, height * width), attn2.permute(0, 2, 1))
        out22 = out22.view(batch_size, channels//4, height, width)
        ...
        out = torch.cat([out11, out21,out12,out22], dim=1)
        out = self.up(out)
        out = self.ptconv(out)
        # Scale and apply gamma
...
class MTB_dsc_2bcam_uom(nn.Module):
    def __init__(self,
                 in_channels: int = 1,
                 num_classes: int = 2,
                 bilinear: bool = True,
                 base_c: int = 64):
        super(MTB_dsc_2bcam_uom, self).__init__()
        self.in_channels = in_channels
        self.num_classes = num_classes
        self.bilinear = bilinear
...
#layer2
        ...
        x2 = self.cross_attention1(torch.cat([x12, x22], dim=1))
        x2 = self.ptconv1(x2)#`4base --- 2base`
        midlayer.append(x2)
#layer3
        x12 = self.down2(x12)
...
#layer4
        ...
        # x14 = self.conv3(torch.cat([x14, x14x, x14y], dim=1))#256
        x23 = self.down3(x23)
        x24 = self.doubleconv3(x23)
        # x24x = self.conv3x(x23)
        # x24y = self.conv3y(x23)
        # x24 = self.conv3(torch.cat([x24, x24x, x24y], dim=1))#256
