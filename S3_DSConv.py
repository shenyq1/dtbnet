...
#构造 initialize: 构造方法初始化一些基本参数和形状信息。
# 生成坐标图: _coordinate_map_3D 方法计算出新的坐标点，并将其应用于输入特征图。
# 双线性插值: _bilinear_interpolate_3D 方法将在新的坐标处对特征图进行双线性插值，生成变形的特征图。
# 返回结果: deform_conv 最终返回处理后的变形特征图。
